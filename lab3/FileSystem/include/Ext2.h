...
struct ext2_inode {
    ...
    uint16_t i_block[8];	// 指向数据块的指针 --> [0,5](直接索引) [6](一级间接) [7](二级间接)
...
public:
    ...
    // search
    uint16_t searchFile(char const tar[9], char file_type, uint16_t& inode_idx, uint16_t& block_idx, uint16_t& dir_idx);
